...
class FeedStore {
    ...
    func insert(_ items: [FeedItem], timesStamp: Date) {
        insertCallCount += 1
        insertions.append((items, timesStamp))
    }
...
class LocalFeedLoader {
    ...
    func save(_ items: [FeedItem]) {
        store.deleteCachedFeed { [unowned self] error in
            if error == nil {
                self.store.insert(items, timesStamp: self.currentDate())
            }
        }
    }
}
...
final class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_requestsInsertionOnSuccessfulDeletion() {
        let (sut, store) = makeSUT()
        let items = [uniqueItem(), uniqueItem()]
