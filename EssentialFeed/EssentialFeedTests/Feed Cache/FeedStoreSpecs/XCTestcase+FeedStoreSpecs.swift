...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatRetrieveDeliversFoundValuesOnNonEmptyCache(
        ...
        expect(sut, toRetrive: .success(CahedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatRetrieveHasNoSideEffectsOnNonEmptyCache(
        ...
        expect(sut, toRetriveTwice: .success(CahedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatInsertDeliversNoErrorOnNonEmptyCache(
        ...
        let insertionError = insert((uniqueImageFeed().local, Date()), to: sut)
        XCTAssertNil(insertionError, "Expected feed to override cache successfully", file: file, line: line)
    }
    ...
    func assertThatInsertOverridesPreviouslyInsertedCacheValues(
        ...
        expect(sut, toRetrive: .success(CahedFeed(feed: latestFeed, timestamp: latestTimestamp)), file: file, line: line)
    }
    ...
    func assertThatSideEffectsRunSerially(
        ...
        XCTAssertEqual(completedOperationsInOrder, [op1, op2, op3], "Expected side-effects to run in order but operations finished in the wrong order", file: file, line: line)
    }
    ...
    func insert(
        ...
        sut.insert(cache.feed, timesStamp: cache.timestamp) { receivedInsertionResult in
    ...
    func expect(
        _ sut: FeedStore,
        toRetrive expectedResult: FeedStore.RetrivalResult,
        file: StaticString = #filePath,
        line: UInt = #line
    ) {
        ...
        sut.retrieve { retrievedResult in
            switch (retrievedResult, expectedResult) {
                ...
                case let (.success(.some(expectedFeed)), .success(.some(retrievedFeed))):
                    ...
                    XCTAssertEqual(expectedFeed.timestamp, expectedFeed.timestamp, file: file, line: line)
    ...
    func expect(
        _ sut: FeedStore,
        toRetriveTwice expectedResult: FeedStore.RetrivalResult,
        file: StaticString = #filePath,
        line: UInt = #line
    ) {
